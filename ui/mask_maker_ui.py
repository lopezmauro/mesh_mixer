...
class MaskWidget(QtWidgets.QWidget):
    ...
    def __init__(self, mixer, parent=None):
        ...
        self.refresh_btn.clicked.connect(self.load_mask_list)
    ...
    def create_ui(self):
        ...
        # Refresh button
        self.refresh_btn = QtWidgets.QPushButton("Load Masks from selection")
    ...
    def refresh_mask_list(self):
        ...
        has_masks = bool(self.mask_list.count())
        self.has_masks_Signal.emit(has_masks)
        if has_masks:
            self.normalize_masks()
    ...
    def load_mask_list(self):
        """Load the list of masks from the selected mesh"""
        # Get the selected mesh
        selection = cmds.ls(selection=True)
        if not selection:
            self.status_label.setText("Error: No mesh selected")
            return
        shape_node = selection[0]
        # Check if the selected object is a mesh
        if cmds.nodeType(shape_node) != "mesh":
            shapes = cmds.listRelatives(shape_node, shapes=True, ni=True)
            if not shapes or cmds.nodeType(shapes[0]) != "mesh":
                self.status_label.setText("Error: No mesh found")
                return
            shape_node = shapes[0]
        transform_node = cmds.listRelatives(shape_node, parent=True)[0]
        shape_node = self.mixer.set_mask_mesh(transform_node)
